...
// Using ShieldGuard Theme Colors for the Tunnel
#define POINT_COLOR_A vec3(0.0, 0.666, 1.0) // var(--blue)
#define POINT_COLOR_B vec3(0.91, 0.125, 0.235) // var(--red)
#define SPEED 0.7
...
void main(){
  ...
  float pointSize = (POINT_SIZE * 1.5) / (2.0 * iResolution.y); // Slightly increased point size
  ...
  for(int i = 1; i <= TUNNEL_LAYERS; i++){
    ...
    if(abs(length(uv + offs) - ringRad) < pointSize * 1.5){
      ...
      vec3 ptColor = (mod(float(i/4), 2.0) == 0.0) ? POINT_COLOR_A : POINT_COLOR_B;
      float shade = (1.0 - pz);
...
export function TunnelBackground({ children }) {
  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden flex items-center justify-center bg-[var(--bg)]">
      <canvas
        ref={canvasRef}
        className="fixed top-0 left-0 w-full h-full pointer-events-none"
        style={{ zIndex: 0 }}
      />
      {/* Radial overlay to blend tunnel edges naturally with page background */}
      <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_center,transparent_30%,var(--bg)_100%)] z-0" />
